...
mod imp {
    ...
    pub struct CircularProgressPaintable {
        pub widget: RefCell<Option<gtk::Image>>,
    ...
    impl SymbolicPaintableImpl for CircularProgressPaintable {
        fn snapshot_symbolic(
            ...
            let bounds =
                graphene::Rect::new(-2.0, -2.0, (width + 4.0) as f32, (width + 4.0) as f32);
    ...
    impl CircularProgressPaintable {
        ...
        pub fn set_widget(&self, widget: gtk::Image) {
            let obj = self.obj().clone();
            self.widget.replace(Some(widget.clone()));
            widget.connect_notify_local(Some("scale-factor"), move |_, _| {
                obj.invalidate_size();
            });
        }
